...
$sql1="insert into order_main(user_name,shipping_address,total_amount,order_date,order_status,last_update_date) 
values('$usr','$adr','$amnt',now(),'Pending',now())";
...
while($row=mysqli_fetch_array($rsCart))
{
   ...
   $rt=$row["item_rate"] ;
   mysqli_query($con,"insert into order_detail(item_id,item_rate,item_quantity,order_main_id) 
   values('$itm','$rt','$qty','$mid')");
}
